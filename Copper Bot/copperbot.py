...
def mine(oretype,queuexy=None):
    ...
    while orecount(oretype) == orig and attempts<=20:
        ...
        if oldmousepos[0]-30<=xtest<=oldmousepos[0]+30 and oldmousepos[1]-30<=ytest<=oldmousepos[1]+30:
            x,y = colormatch(oretype,move="yes")
            humanmovexy(x,y)
...
def colormatch(oretype,move="no"):
    ...
    if oretype == "copper":
        ...
        rgblist = (64, 34, 23), (61, 31, 22), (47, 25, 15), (34, 17, 12), (50, 26, 17), (43, 22, 14), (30, 15, 10), (57, 30, 20), (26, 14, 9), (39, 20, 14)
    for i in range(170000):
        rndx = randint(gamewindow[0],gamewindow[0] + gamewindow[2])
        rndy = randint(gamewindow[1],gamewindow[1] + gamewindow[3])
        if rndx%2==0:
            for t in rgblist:
                if (pyautogui.pixelMatchesColor(rndx,rndy,t,tolerance=5)):
                    print("match at " + str(rndx) + " " + str(rndy))
                    rndx+=randint(-2,2)
                    rndy+=randint(-2,2)
